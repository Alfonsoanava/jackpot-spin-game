...
interface IERC20JPPrize {
    function balanceOf(address account) external view returns (uint256);
    function transfer(address to, uint256 amount) external returns (bool);
}

/// @title JP Prize Claims
/// @notice Owner-approved Jackpot Spin prize claims paid in JP on Base.
/// @dev Keep paused and fund with only a small test batch until testing is complete.
contract JPPrizeClaims {
    IERC20JPPrize public constant JP =
        IERC20JPPrize(0xb200000000000000000000d30ab577FdEa25D940);

    uint256 public constant TEN_JP = 10 ether;
    uint256 public constant TWENTY_FIVE_JP = 25 ether;
    uint256 public constant FIFTY_JP = 50 ether;
    ...
    function validAmount(uint256 amount) public pure returns (bool) {
        return (
            amount == TEN_JP ||
            amount == TWENTY_FIVE_JP ||
            amount == FIFTY_JP
        );
    }

    function contractBalance() external view returns (uint256) {
        return JP.balanceOf(address(this));
    }
        ...
        returns (bool)
    {
        ...
        return (
            ...
            JP.balanceOf(address(this)) >= prize.amount
        );
    }
    ...
    function claimPrize(bytes32 claimId) external nonReentrant {
        ...
        if (JP.balanceOf(address(this)) < prize.amount) {
    ...
    function _safeTransfer(address to, uint256 amount) private {
        (bool success, bytes memory data) = address(JP).call(
            abi.encodeWithSelector(IERC20JPPrize.transfer.selector, to, amount)
        );
